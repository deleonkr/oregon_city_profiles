...
getORGeos <- function(level = "town") {
  ...
  plc_fips <- fips[c(2:63),]
  plc_fips$county <- sapply(plc_fips$FIPS, function(x) {
    as.numeric(substr(x,4,5))
  })
  countyfips <- unique(plc_fips$county)
  
  if (returnTowns) { # If we're return towns (or both), process place fips into separate column
    plc_fips$plc <- sapply(plc_fips$FIPS, function(x) {
      as.numeric(substr(x,6,10))
    })
  }
  
  # Generate ACS structured Geo Object
  # no matter what, start with geo object that contains one entry for the state level
  geosObj <- geo.make(state=41)
  ...
  for(i in 1:length(countyfips)) {
    ...
    if (returnTowns) { # If we're return towns (or both)
      places <- plc_fips[plc_fips$county==county,4]
      geos <- geo.make(state=41, place=places$plc)
      geosObj <- geosObj + geos
    }
    #if (returnCounties) { # If we're return counties (or both)
      #geos <- geo.make(state = 41, county = county)
      #geosObj <- geosObj + geos
   # }
  }
  return(geosObj)
...
getACSData <- function(geoObject, yearList, table) {
  ...
  for (y in yearList) {
    ...
    filename <- paste("ACS", (as.numeric(y)-2000), "5YR", "B15002", "with", "ann.csv", sep = "_")
    rawFile <- file.path(getwd(), "raw", filename)
    if (file.exists(rawFile)) {
      print("Found raw file")
      #data <- read.acs(rawFile, endyear = y, geocols = 1:3)
      data <- read.csv("raw/ACS_21_5YR_B15002_with_ann.csv")
    } else {
...
getACSFips <- function(acsObject) {
  if (setequal(names(geography(acsObject)), c("Id", "Id2", "Geography"))) {
    ...
    datafips$fips <- paste(
      ...
      ifelse(is.na(datafips$place), "", datafips$place),
      sep=""
    )
  } else {
